...
interface BaseTransitionConfig<T extends object, S extends string | number, M extends object> {
  transitions: Transition<T, S, M>[],
  skippable?: boolean
}

/** Configuration of a single step: the transitions leading away from it and optional data about the step itself. */
interface TransitionConfigWithMeta<T extends object, S extends string | number, M extends object> extends BaseTransitionConfig<T, S, M> {
  meta?: M
}
